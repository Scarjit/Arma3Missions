...
FSG_USE_MAINRADAR = TRUE; //Use a single radar for all CRAMs (Reduces lag by quite alot)
FSG_MAINRADAR_NAME = mainradar; //Use objNull if not using. (Otherwise place radar on map and name it something)
...
FSG_CRAMRANGE = 2500; //CRAM Radar Range
FSG_ENGAGERANGE = 2500; //CRAM Engage Range
FSG_RADAROFFSET = 180; //Incoming must be pointing towards cram in this range to be shot down

FSG_SLEEPCYCLE = 3; //Sleep control for almost everything (Increase if heavy lag)
